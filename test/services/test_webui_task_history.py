...
def test_markdown_script_survives_task_history_restore_payload(tmp_path):
    task_id = "markdown-history"
    ...
    (task_path / "script.json").write_text(
        json.dumps(
            {
                "script": params.markdown_script.script_text(),
    ...
    TASK_HISTORY_NAMESPACE["utils"] = SimpleNamespace(task_dir=lambda: str(tmp_path))

    payload = load_task_restore_payload(task_id)

    restored = VideoParams.model_validate(payload["params"])
    assert restored.markdown_script.title == "婚姻二字"
    assert restored.markdown_script.rows[0].emphasis_terms == ["婚姻"]
    assert payload["params"]["markdown_script"] == markdown_script
