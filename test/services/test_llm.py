...
class TestScriptPromptOptions(unittest.TestCase):
    ...
    def test_generate_emphasis_terms_accepts_only_distinct_terms_from_script(self):
        script = "婚姻不是年轻人逃避吃苦而编出来的段子。"

        with patch.object(
            llm,
            "_generate_response",
            return_value='```json\n["不是年轻人", "逃避吃苦而编", "不是年轻人", "不存在"]\n```',
        ):
            result = llm.generate_emphasis_terms(script)

        self.assertEqual(result, ["不是年轻人", "逃避吃苦而编"])

    def test_generate_emphasis_terms_returns_empty_list_for_invalid_provider_response(self):
        script = "婚姻需要共同经营。"
        ...
        with patch.object(
            ...
            self.assertEqual(llm.generate_emphasis_terms(script), [])
