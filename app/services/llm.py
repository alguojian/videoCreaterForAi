...
def _normalize_emphasis_term(value: str) -> str:
    """Return a comparable emphasis term without spacing or punctuation."""
    return re.sub(r"[\W_]+", "", str(value or ""), flags=re.UNICODE)


def generate_emphasis_terms(video_script: str) -> List[str]:
    """Extract short, visible emphasis terms without making video generation fragile.

    The returned terms are always validated against the narration. Invalid model
    output deliberately becomes an empty list, so the deterministic subtitle
    fallback in ``app.services.emphasis`` can take over.
    """
    normalized_script = _normalize_emphasis_term(video_script)
    if len(normalized_script) < 2:
        return []

    prompt = f"""
# Role: Short-video emphasis term extractor

## Goal
Choose up to 8 short phrases from the video script that deserve a visual emphasis overlay.

## Constraints
1. Return only a JSON array of strings, with no markdown or commentary.
2. Every string must occur verbatim in the video script after whitespace and punctuation are ignored.
3. Each string must contain 2 to 16 visible characters. Prefer meaningful phrases instead of isolated characters.
4. Do not repeat terms and do not invent or paraphrase text.
...
""".strip()
    ...
    try:
        ...
        if not isinstance(response, str) or response.startswith("Error: "):
            logger.warning("failed to generate emphasis terms: provider returned an error")
            return []
        payload = json.loads(_strip_code_fence(response))
    except Exception as exc:
        logger.warning(f"failed to generate emphasis terms: {exc}")
        return []

    if not isinstance(payload, list):
        logger.warning("failed to generate emphasis terms: response is not a list")
        return []
    ...
    for item in payload:
        if not isinstance(item, str):
            continue
        term = _normalize_emphasis_term(item)
        if (
            2 <= len(term) <= 16
            and term in normalized_script
    ...
    logger.success(f"completed emphasis terms: {terms}")
    return terms
