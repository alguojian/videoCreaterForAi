...
def _infer_tts_server_from_voice(voice_name):
    ...
    if str(voice_name or "").strip().lower().startswith("local:"):
...
def _clear_markdown_import_state():
    for key in (
...
def _apply_pending_task_restore():
    ...
    st.session_state["emphasis_sfx_volume_slider"] = min(
        0.5, max(0.0, float(params.get("emphasis_sfx_volume", 0.5)))
    )
    ...
    with panel:
        with st.container(border=True):
            ...
            if uploaded_markdown is not None:
                ...
                if markdown_hash != st.session_state.get("markdown_script_hash"):
                    st.session_state["markdown_script_hash"] = markdown_hash
                    try:
                        document = script_document.parse_markdown_upload(
                            uploaded_markdown.name,
                            markdown_payload,
                        )
                    except script_document.MarkdownScriptError as exc:
                        st.session_state["markdown_script_document"] = None
                        st.session_state["markdown_script_error"] = str(exc)
                    else:
            ...
            if serialized_document:
                try:
                    markdown_document = (
                        script_document.MarkdownScriptDocument.model_validate(
                            serialized_document
                        )
                    )
            ...
            if markdown_import_active:
                ...
                row_metric.metric(tr("Markdown Script Rows"), len(markdown_document.rows))
                scene_metric.metric(tr("Markdown Scenes"), len(scenes))
                ...
                scene_rows = [
                    {
                        ...
                        tr("Markdown Scene Search Terms"): "；".join(
                            scene.search_terms
                        ),
                    }
                    for scene in scenes
            ...
            if markdown_error:
                st.error(f"{tr('Markdown Parse Error')}: {markdown_error}")
            if markdown_import_active or markdown_error:
                st.button(
                    tr("Clear Markdown Import"),
                    key="clear_markdown_import",
...
def _render_video_settings(panel, params):
    ...
    with panel:
        with st.container(border=True, key="video_settings"):
            ...
            # 所有新任务默认生成 16:9 横版；仍使用 source-specific widget key
            # 保留用户对单个素材源显式选择竖版的历史状态。
            #   - 用户在某 source 下手动改过 aspect,session_state 会记住,
            #     下次回到同一 source 时尊重用户选择,不会再被强制覆盖。
            default_aspect_index = 1
    ...
    with st.spinner(tr("Synthesizing Voice")):
        ...
        try:
            with config.runtime_config_lock():
                if selected_tts_server == "local-cosyvoice":
                    if not config.local_voice.get("enabled", False):
                        st.error("Local CosyVoice is disabled. Set [local_voice].enabled = true in config.toml.")
                        return
                    local_preview_dir = os.path.join(
                        temp_dir, f"local-voice-preview-{str(uuid4())}"
                    )
                    settings = local_voice_service.settings_from_config(
                        dict(config.local_voice), root_dir
                    )
                    result = local_voice_service.LocalVoiceService(settings).synthesize(
                        f"preview-{str(uuid4())}",
                        local_preview_dir,
                        play_content,
                        voice_name,
            ...
            logger.error(f"local voice preview failed: {exc}")
            st.error(f"Local CosyVoice preview failed: {exc}")
            if local_preview_dir:
                shutil.rmtree(local_preview_dir, ignore_errors=True)
            return
    ...
    with panel:
        with st.container(border=True):
            ...
            if tts_mode_enabled:
                ...
                if selected_tts_server == "local-cosyvoice":
                    st.info(
                        "Windows native local CosyVoice3. "
                        "Use local:default or a profile under resource/voice_profiles."
                    )
            ...
            if not tts_mode_enabled:
                ...
                filtered_voices = local_voice_service.LocalVoiceService(
                    settings
                ).available_voice_names()
                if not config.local_voice.get("enabled", False):
                    st.warning(
                        "Local CosyVoice is selected but disabled. Set [local_voice].enabled = true in config.toml."
                    )
            else:
                # 获取Azure的声音列表
            ...
            def _friendly(v):
                ...
                if str(v).startswith("local:"):
                    profile_id = v.split(":", 1)[1] or "default"
                    return f"Local voice: {profile_id}"
                return (
...
def _render_emphasis_settings(panel, params):
    """渲染任务级的重点词文字与固定提示音设置。"""
    with panel:
        with st.container(border=True):
            ...
            with settings_columns[0]:
                st.session_state.setdefault("emphasis_random_colors_checkbox", True)
                params.emphasis_random_colors = st.checkbox(
                    tr("Random Emphasis Colors"),
                    key="emphasis_random_colors_checkbox",
                    disabled=disabled,
                )
            with settings_columns[1]:
                ...
                params.emphasis_random_animations = st.checkbox(
                    ...
                    key="emphasis_random_animations_checkbox",
                    disabled=disabled,
                )

            st.session_state.setdefault("emphasis_sfx_enabled_checkbox", True)
            params.emphasis_sfx_enabled = st.checkbox(
                tr("Enable Emphasis Sound Effects"),
                key="emphasis_sfx_enabled_checkbox",
                disabled=disabled,
            )
            st.session_state.setdefault("emphasis_sfx_volume_slider", 0.5)
            params.emphasis_sfx_volume = st.slider(
                tr("Emphasis Sound Volume"),
                0.0,
                0.5,
                step=0.01,
                key="emphasis_sfx_volume_slider",
                disabled=disabled or not params.emphasis_sfx_enabled,
            )
...
def _render_generation_controls(
    ...
    markdown_source_invalid = bool(
        params.markdown_script
        and params.video_source not in {"pexels", "pixabay", "coverr"}
    )
...
def _render_application():
    ...
    _render_emphasis_settings(right_panel, params)
